<template>
  <vk-group>
    <vk-circle :config="{ x: 310, y: 310, width: mainStore.samParams.MAX_DISTANCE / (scale), fill: 'rgb(15, 33, 19)' }" />
    <!-- Distance circles -->
    ...
    <vk-circle
      :config="{ x: 310, y: 310, width: mainStore.samParams.MAX_CAPTURE_RANGE / scale, stroke: 'rgb(150, 249, 123)', dash: [2, 5], strokeWidth: 1.5 }" />
    <!-- Min capture range circle -->
    <vk-circle
      :config="{ x: 310, y: 310, width: mainStore.samParams.MIN_CAPTURE_RANGE / scale, stroke: 'rgb(150, 249, 123)', dash: [2, 5], strokeWidth: 1.5 }" />
    <!-- Killzone circle -->
    <vk-circle
      :config="{ x: 310, y: 310, width: mainStore.samParams.MISSILE_MAX_DISTANCE / scale, stroke: 'red', strokeWidth: 0.5 }" />
    <!-- Azimut lines -->
    <vk-line :config="{
...
const countCircles = computed(() => mainStore.samParams.MAX_DISTANCE / 10000);
const azimutLines = computed(() => {
  return Array(36).fill(0).map((_, i) => {
    ...
    return {
      ...
      x1: cos * (mainStore.samParams.MAX_DISTANCE / (props.scale * 2)) + 310,
      y1: sin * (mainStore.samParams.MAX_DISTANCE / (props.scale * 2)) + 310,
