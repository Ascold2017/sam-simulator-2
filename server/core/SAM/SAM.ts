...
export class SAM {
  ...
  private updateRadar() {
    ...
    const detectedEnemys = enemys
      .filter((e) => {
        ...
        return distance < SAM_PARAMS.MAX_CAPTURE_RANGE &&
          distance > SAM_PARAMS.MIN_CAPTURE_RANGE;
      })
    ...
    this.radarObjects = [
      ...
      ...undetectedEnemys.map((fo) => new UndetectedRadarObject(fo)).filter(
        (fo) => fo.isVisible
      ),
  ...
  public selectTarget(targetId: string) {
    ...
    if (
      radarObject && !this.selectedObjectIds.some((id) => id === targetId) &&
      this.selectedObjectIds.length < SAM_PARAMS.RADAR_MAX_SELECTED_COUNT
    ) {
