...
export default class BaseRadarObject {
  ...
  constructor(payload: BaseRadarObjectConstructor) {
    ...
    const inAllowedElevation = this.elevation > SAM_PARAMS.MIN_ELEVATION &&
      this.elevation < SAM_PARAMS.MAX_ELEVATION;
    this.isVisible = this.isInVision(distance, payload.currentPoint.z) &&
      inAllowedElevation && distance < SAM_PARAMS.MAX_DISTANCE;
  }

  protected isInVision(distance: number, height: number) {
    return Math.sqrt(2 * 6371009 * SAM_PARAMS.RADAR_HEIGHT) +
